...
This may be my slowest program that solves the problem using a "fast" method. Other contenders are my Kitten program (2023, Day 11) or solving by hand (2023, Day 6). Judging by the pull requests on GitHub, version 0.6 of Amber is likely to support recursion, so I might eventually re-implement this solution and see if it runs any faster. (It might also solve the weird bug of the program not working without the do-nothing `join(args, ",")` statement on line 52...)
